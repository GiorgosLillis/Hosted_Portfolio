...
async function init() {
    ...
    try {
        ...
        if (freshWeather && freshWeather.current) {
            ...
            CurrentWeatherInfo(freshWeather.current.temperature, freshWeather.current.condition, freshWeather.current.icon, freshWeather.time, freshWeather.current.img);
        } else {
