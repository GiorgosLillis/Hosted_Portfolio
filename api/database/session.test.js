...
// functions.js and session.js each call Redis.fromEnv() independently -
// functions.js's instance (used internally by checkToken) is created first since it's
// evaluated as a dependency before session.js's own top-level code runs.
...
describe('session handler', () => {
    ...
    describe('GET (logout current device)', () => {
        it('clears the auth cookies and returns 200', async () => {
            ...
            const setCookieCall = res.setHeader.mock.calls.find(call => call[0] === 'Set-Cookie');
            expect(setCookieCall[1][0]).toMatch(/^token=;/);
        });
    });

    describe('POST (logout other devices)', () => {
